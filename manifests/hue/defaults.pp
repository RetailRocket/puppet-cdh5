...
class cdh5::hue::defaults {
    ...
    # set in the cdh5::oozie class.
    if (defined(Class['cdh5::oozie'])) {
        $oozie_url                = $cdh5::oozie::url
    ...
    $ssl_private_key          = undef #'/etc/ssl/private/hue.key'
    $ssl_certificate          = undef #'/etc/ssl/certs/hue.cert'
    ...
    $hue_ini_template         = 'cdh5/hue/hue.ini.erb'
